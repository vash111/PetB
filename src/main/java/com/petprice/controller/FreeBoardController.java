...
public class FreeBoardController {
    ...
    // 게시글 작성
    @PostMapping
    public ResponseEntity<FreeBoardDTO> createPost(@RequestBody FreeBoardDTO dto, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).build();
        }
        return ResponseEntity.ok(freeBoardService.createPost(dto, user));
    }

...
public class FreeBoardService {
    ...
    // 게시글 작성
    @Transactional
    public FreeBoardDTO createPost(FreeBoardDTO dto, User author) {
        FreeBoard freeBoard = new FreeBoard();
        freeBoard.setTitle(dto.getTitle());
        freeBoard.setContent(dto.getContent());
        freeBoard.setCategory(dto.getCategory()); // category 설정
        freeBoard.setAuthor(author);
        FreeBoard savedPost = freeBoardRepository.save(freeBoard);
        return convertToDTO(savedPost, author); // 현재 사용자 전달
    }
    ...
    public List<FreeBoardDTO> getPostsByUserId(Long userId) {
        return freeBoardRepository.findByAuthorId(userId).stream()
                .map(post -> convertToDTO(post, null)) // 현재 사용자 정보는 필요 없음
                .collect(Collectors.toList());
    }
